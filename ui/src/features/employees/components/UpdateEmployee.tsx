...
import { DropdownOption, FormData, FormErrors, Permission } from '../types';
...
const UpdateEmployee = () => {
        ...
        try {
            ...
            if (response.success) {
                ...
                setFormData({
                    ...
                    reportsToId: employee.reportsTo?.id || null,
                    permissions: employee.permissions.map(p => p.name),
                    profilePhoto: undefined,
                });
                setCurrentProfilePhoto(
                    employee.profileAsset?.cloudFrontUrl || null
                );
            } else {
                toast.error(l('employees.messages.failedToFetchEmployee'));
                navigate('/employees');
            }
        } catch (error) {
            toast.error(l('employees.messages.failedToFetchEmployee'));
        ...
        try {
            ...
            if (response.success) {
                const employeeOptions: DropdownOption[] = (response.data || [])
                    .filter(employee => employee.id !== Number(id))
                    .map(employee => ({
    ...
    const validateForm = (): boolean => {
        ...
        if (!formData.firstName.trim()) {
            newErrors.firstName = l('employees.validation.firstNameRequired');
        }

        if (!formData.lastName.trim()) {
            newErrors.lastName = l('employees.validation.lastNameRequired');
        }

        if (!formData.email.trim()) {
            newErrors.email = l('employees.validation.emailRequired');
        } else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(formData.email)) {
            newErrors.email = l('employees.validation.emailInvalid');
        }

        if (formData.password && formData.password.length < 8) {
            newErrors.password = l('employees.validation.passwordMinLength');
        }

        if (!formData.position.trim()) {
            newErrors.position = l('employees.validation.positionRequired');
        }

        if (!formData.phone.trim()) {
            newErrors.phone = l('employees.validation.phoneRequired');
        } else if (!/^\+[1-9]\d{1,14}$/.test(formData.phone)) {
            newErrors.phone = l('employees.validation.phoneInvalid');
        }

        if (!formData.departmentId) {
            newErrors.departmentId = l(
                'employees.validation.departmentRequired'
            );
        }

        if (!formData.locationId) {
            newErrors.locationId = l('employees.validation.locationRequired');
        }
    ...
    const handlePhotoSelect = (file: File | null) => {
        setFormData(prev => ({
            ...
            removeProfilePhoto: file === null ? true : undefined,
        }));
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (response.success) {
                toast.success(l('employees.messages.updateSuccess'));
                navigate('/employees');
            } else {
                toast.error(
                    response.message || l('employees.messages.updateFailed')
                );
            }
        } catch (error: any) {
            toast.error(error.message || l('employees.messages.updateFailed'));
        } finally {
            setLoading(false);
        }
    };
            ...
            <Card className="p-4">
                <form onSubmit={handleSubmit} className="space-y-6">
                        ...
                        <div>
                            <FloatLabel className={floatLabelClassName}>
                                <Password
                                    ...
                                    onChange={handleInputChange}
                                    className="w-full"
                                    inputClassName={`${inputClassName} !pl-3`}
                                    feedback={false}
                                    toggleMask
                                />
                                <label
                                ...
                                >
                                    {l('employees.fields.password')} (Leave
                        ...
                        <div>
                            <FloatLabel className={floatLabelClassName}>
                                ...
                                <label
                                    ...
                                    className="transition-all duration-200"
                                >
                                    {l('employees.fields.phone')} (E.164) *
                                </label>
                            </FloatLabel>
                            {errors.phone && (
                                <small className="text-red-500">
                    ...
                    <div className="flex justify-end gap-3 pt-4">
                        ...
                        <Button
                            type="submit"
                            label={l('employees.actions.edit')}
                            loading={loading}
